...
fun CarePlanJpa.toDomain() = CarePlan(
    id = CarePlanId(id), version = version, scheduleRevision = scheduleRevision, tutorId = TutorId(tutorId), petId = PetId(petId),
    ...
    reminderMinutesBefore = reminderMinutesBefore, active = active, createdAt = createdAt, updatedAt = updatedAt,
)

fun CarePlan.toJpa() = CarePlanJpa().also {
    it.id = id.value; it.version = version; it.scheduleRevision = scheduleRevision; it.tutorId = tutorId.value; it.petId = petId.value
    ...
    it.reminderMinutesBefore = reminderMinutesBefore; it.active = active; it.createdAt = createdAt; it.updatedAt = updatedAt
}

fun CareOccurrenceJpa.toDomain() = CareOccurrence(
    id = CareOccurrenceId(id), version = version, planId = CarePlanId(planId), scheduleRevision = scheduleRevision, tutorId = TutorId(tutorId),
    petId = PetId(petId), sequence = sequence, type = type, title = title, instructions = instructions,
    dueAt = dueAt, status = status, completedAt = completedAt,
    completedByTutorId = completedByTutorId?.let(::TutorId), completionNote = completionNote,
    createdAt = createdAt, updatedAt = updatedAt, legacyEventId = legacyEventId,
)

fun CareOccurrence.toJpa() = CareOccurrenceJpa().also {
    it.id = id.value; it.version = version; it.planId = planId.value; it.scheduleRevision = scheduleRevision; it.tutorId = tutorId.value; it.petId = petId.value
    ...
    it.completionNote = completionNote; it.createdAt = createdAt; it.updatedAt = updatedAt; it.legacyEventId = legacyEventId
}
